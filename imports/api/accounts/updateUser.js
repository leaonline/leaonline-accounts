 ...
 * @returns {Promise<number>} the number of updated documents
 */
export const updateUser = async (update, original, debug = () => {}) => {
	...
	const rolesToRemove = currentRoles.filter(
		(role) => institutionChanged || !update.roles.includes(role),
	)
	if (rolesToRemove.length > 0) {
		...
		const removed = await removeRoles(
			original._id,
			rolesToRemove,
		...
		if (!updateRequired) {
			throw new Meteor.Error(
				'updateUser.error',
				'updateUser.removeRoleFailed',
				{ rolesToRemove, removed, verified },
			)
		}
	...
	if (rolesToAdd.length > 0) {
		...
		if (!updateRequired) {
			...
			throw new Meteor.Error(
				'updateUser.error',
				'updateUser.assignRoleFailed',
				{ rolesToAdd, assigned, verified },
			)
		}
